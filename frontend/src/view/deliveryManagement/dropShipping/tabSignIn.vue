<template>
  ...
  >
    <vxe-table ref="xTable" :column-config="{ minWidth: '100px' }" :data="data.tableData" :height="tableHeight" align="center">
      ...
      <vxe-column field="waybill_no" :title="$t('wms.dropShipping.waybill_no')"></vxe-column>
      <vxe-column field="carrier" :title="$t('wms.dropShipping.carrier')"></vxe-column>
      <vxe-column field="damagedQuantity" :title="$t('wms.dropShipping.damagedQuantity')">
        <template #default="{ row }">
...
import { getSignIn } from '@/api/wms/dropShipping'
...
const method = reactive({
  ...
  exportTable: () => {
    ...
    exportData({
      table: $table,
      filename: i18n.global.t('wms.dropShipping.signedIn'),
