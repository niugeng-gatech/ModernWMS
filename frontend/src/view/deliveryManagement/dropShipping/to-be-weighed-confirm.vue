  ...
  <v-dialog v-model="isShow" :width="'30%'" transition="dialog-top-transition" :persistent="true">
    <template #default>
      <v-card class="formCard">
        <v-toolbar color="white" :title="`${$t('wms.dropShipping.weigh')}`"></v-toolbar>
        <v-card-text>
          <v-form ref="formRef">
            <v-text-field
            ...
            <v-text-field
              ...
              :label="$t('wms.dropShipping.detailWeight') + '(' + weightUnit + ')'"
  ...
  submit: () => {
    if (
      ...
      hookComponent.$message({
        type: 'error',
        content: `${ i18n.global.t('wms.dropShipping.validQtyMsgPackage') }${ props.maxQty }!`
      })
    }
  }
})
