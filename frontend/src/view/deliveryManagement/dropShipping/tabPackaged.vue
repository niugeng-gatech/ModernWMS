<template>
  ...
  >
        ...
        visibleMethod: method.getCheckBoxDisableState
          ...
          <vxe-column :title="$t('wms.dropShipping.state')">
              <template #default="{ row }">
                  <span>{{ `${row.is_todo ? $t('wms.dropShipping.packageTodo') : $t('wms.dropShipping.packageReady')}` }}</span>
              </template>
          </vxe-column>
          ...
          <vxe-column field="package_time"
                      width="170px"
                      :formatter="['formatDate', 'yyyy-MM-dd HH:mm']"
                      :title="$t('wms.dropShipping.package_time')"></vxe-column>
    ...
    <PackageConfirm ref="PackageConfirmRef" :dialog-title="$t('wms.dropShipping.package')" :is-weight="false" @submit="method.dialogSubmit" />
  </div>
</template>
...
import { getPackaged, cancelOrderByDetail, handlePackage } from '@/api/wms/dropShipping'
...
const method = reactive({
  ...
  exportTable: () => {
    ...
    exportData({
      table: $table,
      filename: i18n.global.t('wms.dropShipping.packaged'),
