  ...
  <v-dialog v-model="isShow" transition="dialog-top-transition" :persistent="true">
    <template #default>
      <v-card class="formCard" :width="dialogWidth">
        <v-toolbar color="white" :title="`${$t('router.sideBar.dropShipping')}`"></v-toolbar>
        <v-card-text>
          <v-form ref="formRef">
            <!-- <v-select
            ...
            <v-row v-for="(item, index) of data.form.detailList" :key="index" style="margin-top: 5px">
              <!-- <v-select
                  ...
                  :label="$t('wms.dropShipping.sku_code')"
                  variant="outlined"
                  clearable
                ></v-select> -->
              <v-col :cols="3">
                <v-text-field v-model="item.spu_name" :label="$t('wms.dropShipping.spu_name')" variant="outlined" readonly></v-text-field>
              </v-col>
              <v-col :cols="3">
                <v-text-field
                  v-model="item.spu_code"
                  :label="$t('wms.dropShipping.spu_code')"
              ...
              <v-col :cols="3">
                <v-text-field v-model="item.sku_code" :label="$t('wms.dropShipping.sku_code')" variant="outlined" readonly></v-text-field>
              </v-col>
              <v-col :cols="2">
                <v-text-field
                  v-model="item.qty"
                  :rules="data.rules.qty"
                  :label="$t('wms.dropShipping.detailQty')"
                  variant="outlined"
                  clearable
                ></v-text-field>
              </v-col>
...
import { DropShippingVO, DropShippingDetailListVO, addRequestVO } from '@/types/DeliveryManagement/DropShipping'
import i18n from '@/languages/i18n'
import { errorColor } from '@/constant/style'
import { hookComponent } from '@/components/system/index'
import { addShipment } from '@/api/wms/dropShipping'
...
const data = reactive({
  ...
  dialogTitle: '',
  form: ref<DropShippingVO>({
    id: 0,
    detailList: []
  }),
  removeDetailList: ref<DropShippingDetailListVO[]>([]),
  combobox: ref<{
    customer_name: {
      label: string
      value: number
  ...
  rules: {
    customer_name: [
      (val: string) => !!val || `${ i18n.global.t('system.checkText.mustInput') }${ i18n.global.t('wms.dropShipping.customer_name') }!`
    ],
    spu_code: [(val: string) => !!val || `${ i18n.global.t('system.checkText.mustInput') }${ i18n.global.t('wms.dropShipping.sku_code') }!`],
    qty: [
      (val: number) => !!val || `${ i18n.global.t('system.checkText.mustInput') }${ i18n.global.t('wms.dropShipping.detailQty') }!`,
...
const method = reactive({
  ...
  clearCommodity: (item: DropShippingDetailListVO) => {
    item.sku_id = 0
    item.sku_code = ''
    item.spu_code = ''
    item.spu_name = ''
  ...
  // remove detail
  removeItem: (index: number, item: DropShippingDetailListVO) => {
