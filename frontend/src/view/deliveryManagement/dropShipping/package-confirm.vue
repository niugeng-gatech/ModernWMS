        ...
        <v-card-text>
          <v-form ref="formRef">
            <v-row v-for="(item, index) of data.list" :key="index">
              ...
              <v-col :cols="3">
                <v-text-field v-model="item.spu_code" :label="$t('wms.dropShipping.spu_code')" variant="outlined" readonly></v-text-field>
              </v-col>
              ...
              <v-col :cols="isWeight ? 2 : 3">
                <v-text-field
                  ...
                  :label="isSignIn ? $t('wms.dropShipping.damagedQuantity') : $t('wms.dropShipping.detailQty')"
                  variant="outlined"
                  clearable
                ></v-text-field>
              </v-col>
              <v-col v-if="isWeight" :cols="2">
                <v-text-field
                  ...
                  :label="$t('wms.dropShipping.detailWeight') + '(' + item.weight_unit + ')'"
                  variant="outlined"
                  clearable
                ></v-text-field>
              </v-col>
...
import { ConfirmItem } from '@/types/DeliveryManagement/DropShipping'
    ...
    if (props.isSignIn) {
      ...
      if (verificationFailedList.length > 0) {
        const errMsgStrList = verificationFailedList.map(
          (item) => `${ item.sku_code }: ${ i18n.global.t('wms.dropShipping.exceedingQuantity') }${ item.maxQty }`
        )
        hookComponent.$message({
          type: 'error',
          content: `${ i18n.global.t('wms.dropShipping.invalidValue') }; ${ errMsgStrList.join('; ') }`
        })
      } else {
        hookComponent.$dialog({
          content: `${ i18n.global.t('wms.dropShipping.irreversible') }, ${ i18n.global.t('wms.dropShipping.confirmSignIn') }?`,
          handleConfirm: async () => {
