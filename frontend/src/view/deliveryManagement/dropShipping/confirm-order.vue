  ...
  <v-dialog v-model="isShow" :width="'80%'" transition="dialog-top-transition" :persistent="true">
    <template #default>
      <v-card class="formCard">
        <v-toolbar color="white" :title="`${$t('wms.dropShipping.confirmOrder')}`"></v-toolbar>
        <v-card-text>
          <v-row>
            <v-col :cols="5">
              <vxe-table
              ...
              >
                ...
                <vxe-column field="sku_code" :title="$t('wms.dropShipping.sku_code')">
                  <template #default="{ row }">
            ...
            <v-col :cols="7">
              ...
              >
                ...
                <vxe-column field="qty_available" :title="$t('wms.dropShipping.qty_available')"></vxe-column>
                <vxe-column field="serial_number" :title="$t('wms.stockLocation.serial_number')"></vxe-column>
                <vxe-column field="pick_qty" :title="$t('wms.dropShipping.detailQty')" :edit-render="{}">
                  <template #edit="{ row }">
...
import { ConfirmOrderVO, ConfirmOrderPickListVO } from '@/types/DeliveryManagement/DropShipping'
import i18n from '@/languages/i18n'
import { getConfirmOrderInfoAndStock, confirmOrder } from '@/api/wms/dropShipping'
...
const data = reactive({
  navListOptions: {
    title: i18n.global.t('wms.dropShipping.orderDetail'),
    labelKey: 'sku_code',
    indexKey: 'sku_code',
    indexValue: ''
  },
...
const method = reactive({
  ...
  beforeSubmit: (): boolean => {
    ...
    if (data.treeData.filter((fl) => fl.confirm).length <= 0) {
      hookComponent.$message({
        type: 'error',
        content: i18n.global.t('wms.dropShipping.NoItemSelected')
      })
      return false
    }
    for (const item of data.treeData) {
      if (item.confirm) {
        ...
        if (count > item.qty) {
          ...
          msgList.push(`${ item.sku_code } ${ i18n.global.t('wms.dropShipping.quantityOverflow') } ${ item.qty }`)
          // flag = false
        }
        if (count === 0) {
          ...
          msgList.push(`${ item.sku_code } ${ i18n.global.t('wms.dropShipping.detailQuantityIsZero') }!`)
        }
      }
    }
    // return flag
    ...
    if (orderCount <= 0) {
      hookComponent.$message({
        type: 'error',
        content: i18n.global.t('wms.dropShipping.quantityIsZero')
      })
      return false
    }
    return true
  ...
  submit: async () => {
    ...
    if (method.beforeSubmit()) {
      ...
      hookComponent.$message({
        type: 'success',
        content: i18n.global.t('wms.dropShipping.confirmSuccess')
      })
      emit('saveSuccess')
    }
  },
