<template>
  <div class="operateArea">
    <v-row no-gutters>
      ...
      <v-col cols="3" class="col">
        ...
        <tooltip-btn icon="mdi-car-cog" :tooltip-text="$t('wms.dropShipping.setFreight')" @click="method.setFreight"> </tooltip-btn>
        <tooltip-btn icon="mdi-email-check" :tooltip-text="$t('wms.dropShipping.signIn')" @click="method.handleSignIn"></tooltip-btn> -->
  ...
  >
    <vxe-table ref="xTable" :column-config="{ minWidth: '100px' }" :data="data.tableData" :height="tableHeight" align="center">
      ...
      <vxe-column :title="$t('wms.dropShipping.state')">
        <template #default="{ row }">
          <span>{{ `${row.is_todo ? $t('wms.dropShipping.deliveryTodo') : $t('wms.dropShipping.deliveryReady')}` }}</span>
        </template>
      </vxe-column>
        ...
        <template #default="{ row }">
          <div style="width: 100%; display: flex; justify-content: center">
            ...
            <!-- <tooltip-btn
              ...
              :tooltip-text="$t('wms.dropShipping.signIn')"
    ...
    <SignInConfirm
      ...
      :dialog-title="$t('wms.dropShipping.signIn')"
      :is-weight="false"
      :is-sign-in="true"
      @submit="method.dialogSubmit"
    />
...
import { DropShippingDetailVO, SetCarrierVO, ConfirmItem } from '@/types/DeliveryManagement/DropShipping'
import { PAGE_SIZE, PAGE_LAYOUT, DEFAULT_PAGE_SIZE } from '@/constant/vxeTable'
import { hookComponent } from '@/components/system'
import { getDelivery, handleSignIn, setCarrier, handleDelivery } from '@/api/wms/dropShipping'
...
const data = reactive({
  ...
  dialogDefaultQty: 0,
  packageRow: ref<DropShippingDetailVO>(),
  searchForm: {
    dispatch_no: '',
    customer_name: '',
    spu_name: ''
...
const method = reactive({
  ...
  handleSignIn: async () => {
    ...
    checkTableList = checkTableList.filter((item: DropShippingDetailVO) => !item.is_todo)
    ...
    if (checkTableList.length > 0) {
      ...
      hookComponent.$message({
        type: 'error',
        content: `${ i18n.global.t('wms.dropShipping.opeartionCheckboxIsNull') }`
      })
    }
    // data.packageRow = row
  ...
  exportTable: () => {
    ...
    exportData({
      table: $table,
      filename: i18n.global.t('wms.dropShipping.outOfWarehouse'),
  ...
  handleDeliver: async () => {
    ...
    if (checkTableList.length > 0) {
      const deliveredList = checkTableList.map((row: DropShippingDetailVO) => ({
        id: row.id,
      ...
      hookComponent.$dialog({
        content: `${ i18n.global.t('wms.dropShipping.irreversible') }, ${ i18n.global.t('wms.dropShipping.confirmDelivery') }?`,
        handleConfirm: async () => {
...
onMounted(() => {
  data.btnList = [
    ...
    {
      name: i18n.global.t('wms.dropShipping.delivery'),
      icon: 'mdi-cube-send',
      code: 'delivered-delivery',
      click: method.handleDeliver
    },
    {
      name: i18n.global.t('wms.dropShipping.setFreight'),
      icon: 'mdi-car-cog',
      code: 'delivered-setCarrier',
      click: method.setFreight
    },
    {
      name: i18n.global.t('wms.dropShipping.signIn'),
      icon: 'mdi-email-check',
