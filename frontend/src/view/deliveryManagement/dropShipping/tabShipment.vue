<template>
        ...
        <v-row no-gutters @keyup.enter="method.sureSearch">
          ...
          <v-col cols="4">
            <v-select
              ...
              item-title="label"
              item-value="value"
              :label="$t('wms.dropShipping.dispatch_status')"
              variant="solo"
              density="comfortable"
              class="searchInput ml-5 mt-1"
              clearable
  ...
  >
    <vxe-table ref="xTable" :column-config="{ minWidth: '100px' }" :data="data.tableData" :height="tableHeight" align="center">
      ...
      <vxe-column field="dispatch_no" width="120" :title="$t('wms.dropShipping.dispatch_no')"></vxe-column>
      <vxe-column field="dispatch_status" :title="$t('wms.dropShipping.dispatch_status')">
        <template #default="{ row }">
          <span>{{ getShipmentState(row.dispatch_status) }}</span>
        </template>
      </vxe-column>
      <vxe-column field="qty" :title="$t('wms.dropShipping.qty')"></vxe-column>
      <vxe-column field="weight" :title="$t('wms.dropShipping.weight')">
        <template #default="{ row }">
          <span>{{ `${row.weight} ${GetUnit('weight', 2)}` }}</span>
        </template>
      </vxe-column>
      <vxe-column field="volume" :title="$t('wms.dropShipping.volume')">
        <template #default="{ row }">
      ...
      <!-- <vxe-column
        ...
        :title="$t('wms.dropShipping.create_time')"
      ></vxe-column> -->
        ...
        <template #default="{ row }">
          <div style="width: 100%; display: flex; justify-content: center">
            ...
            <tooltip-btn
              ...
              :tooltip-text="$t('wms.dropShipping.confirmOrder')"
            ...
            <tooltip-btn
              ...
              :tooltip-text="$t('wms.dropShipping.confirmPicking')"
    ...
    <qr-code-dialog ref="qrCodeDialogRef" :menu="'dropShipping-shipment'">
      <template #left="{ slotData }">
        <p>{{ $t('wms.dropShipping.dispatch_no') }}:{{ slotData.dispatch_no }}</p> &nbsp;
      </template>
    </qr-code-dialog>
  </div>
</template>
...
import { DropShippingVO } from '@/types/DeliveryManagement/DropShipping'
import { PAGE_SIZE, PAGE_LAYOUT, DEFAULT_PAGE_SIZE } from '@/constant/vxeTable'
import { hookComponent } from '@/components/system'
import { getShipment, delShipment, cancelOrderByDispatch, confirmPicking } from '@/api/wms/dropShipping'
...
const data = reactive({
  ...
  combobox: {
    dispatch_status: [
      {
        label: i18n.global.t('wms.dropShipping.preShipment'),
        value: '0'
      },
      {
        label: i18n.global.t('wms.dropShipping.newShipment'),
        value: '1'
      },
      {
        label: i18n.global.t('wms.dropShipping.goodsToBePicked'),
        value: '2'
      },
      {
        label: i18n.global.t('wms.dropShipping.picked'),
        value: '3'
      },
      {
        label: i18n.global.t('wms.dropShipping.packaged'),
        value: '4'
      },
      {
        label: i18n.global.t('wms.dropShipping.weighed'),
        value: '5'
      },
      {
        label: i18n.global.t('wms.dropShipping.outOfWarehouse'),
        value: '6'
      },
      {
        label: i18n.global.t('wms.dropShipping.signedIn'),
        value: '7'
      }
    ]
  },
...
const method = reactive({
  ...
  // Confirm picking
  confirmPicking: async (row: DropShippingVO) => {
    hookComponent.$dialog({
      content: `${ i18n.global.t('wms.dropShipping.confirmPicking') }?`,
      handleConfirm: async () => {
        if (row.dispatch_no) {
  ...
  backToThePreviousStep(row: DropShippingVO) {
    hookComponent.$dialog({
  ...
  confirmOrder: (row: DropShippingVO) => {
    if (row.dispatch_status === undefined || ![0, 1].includes(row.dispatch_status)) {
      hookComponent.$message({
        type: 'error',
        content: `${ i18n.global.t('wms.dropShipping.incorrectStatusMsg') }${ getShipmentState(0) }${ i18n.global.t(
          'wms.dropShipping.or'
        ) }${ getShipmentState(1) }`
      })
      return
  ...
  exportTable: () => {
    ...
    exportData({
      table: $table,
      filename: i18n.global.t('wms.dropShipping.shipment'),
