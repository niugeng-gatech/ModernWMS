  ...
  <v-dialog v-model="isShow" :width="'30%'" transition="dialog-top-transition" :persistent="true">
    <template #default>
      <v-card class="formCard">
        <v-toolbar color="white" :title="`${$t('wms.dropShipping.setFreight')}`"></v-toolbar>
        <v-card-text>
          <v-form ref="formRef">
            <v-text-field
              v-model="data.form.waybill_no"
              :rules="data.rules.waybill_no"
              :label="$t('wms.dropShipping.waybill_no')"
              variant="outlined"
              clearable
            ></v-text-field>
            <v-text-field
              v-model="data.form.carrier"
              :label="$t('wms.dropShipping.carrier')"
              variant="outlined"
              readonly
              clearable
            ...
            <v-text-field
              ...
              :label="$t('wms.dropShipping.departure_city')"
              variant="outlined"
              readonly
            ></v-text-field>
            <v-text-field
              v-model="data.form.arrival_city"
              :label="$t('wms.dropShipping.arrival_city')"
              variant="outlined"
              readonly
            ></v-text-field>
          </v-form>
  ...
  submit: () => {
    if (data.form.carrier) {
      ...
      hookComponent.$message({
        type: 'error',
        content: `${ i18n.global.t('system.checkText.mustSelect') }${ i18n.global.t('wms.dropShipping.carrier') }!`
      })
    }
  }
})
