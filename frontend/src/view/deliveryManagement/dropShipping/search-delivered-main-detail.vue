  ...
  <v-dialog v-model="isShow" :width="'70%'" transition="dialog-top-transition" :persistent="true">
    <template #default>
      <v-card class="formCard">
        <v-toolbar color="white" :title="`${$t('wms.dropShipping.ViewInventoryDetails')}`"></v-toolbar>
        <v-card-text>
          <vxe-table
            ref="detailXTable"
            keep-source
          ...
          >
            ...
            <vxe-column field="sku_name" :title="$t('wms.dropShipping.sku_name')"></vxe-column>
            <vxe-column field="unit" :title="$t('wms.dropShipping.unit')"></vxe-column>
...
import { hookComponent } from '@/components/system/index'
    import { viewDeliveryMainDetail } from '@/api/wms/dropShipping'
