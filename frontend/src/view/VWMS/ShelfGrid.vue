...
const formatIndex = computed(() => {
  const arr = Array.from({ length: props.shelfGridData.layer * props.shelfGridData.column }, (_, index) => index + 1).reverse()
  const result = []
  for (let i = 0; i < arr.length; i += props.shelfGridData.column) {
    result.push(arr.slice(i, i + props.shelfGridData.column).reverse())
  }
  return result.flat()
})
