    ...
    public class DispatchlistService : BaseService<DispatchlistEntity>, IDispatchlistService
    {
        ...
        public async Task<List<DispatchpicklistViewModel>> GetPickListByDispatchID(int dispatch_id)
        {
            var datas = await (from dpl in _dBContext.GetDbSet<DispatchpicklistEntity>().AsNoTracking()
                               ...
                               select new DispatchpicklistViewModel
                               {
                                   ...
                                   serial_number = dpl.serial_number
                               }).ToListAsync();
            return datas;
        }
        ...
        public async Task<List<DispatchlistConfirmDetailViewModel>> ConfirmOrderCheck(string dispatch_no, CurrentUser currentUser)
        {
            ...
            var stock_group_datas = from stock in stock_DbSet.AsNoTracking()
                                    ...
                                    group stock by new { stock.id, stock.sku_id, stock.goods_location_id, stock.goods_owner_id, stock.serial_number } into sg
                                    select new
                                    {
                                        stock_id = sg.Key.id,
                                        ...
                                        serial_number = sg.Key.serial_number,
            ...
            var dispatch_group_datas = from dp in DbSet.AsNoTracking()
                                       ...
                                       group dpp by new { dpp.sku_id, dpp.goods_location_id, dpp.goods_owner_id, dpp.serial_number } into dg
                                       select new
                                       {
                                           goods_owner_id = dg.Key.goods_owner_id,
                                           sku_id = dg.Key.sku_id,
                                           goods_location_id = dg.Key.goods_location_id,
                                           serial_number = dg.Key.serial_number,
            ...
            var datas = await (from dl in DbSet
                               ...
                               join dp in dispatch_group_datas on new { sg.sku_id, sg.goods_location_id, sg.goods_owner_id, sg.serial_number } equals new { dp.sku_id, dp.goods_location_id, dp.goods_owner_id, dp.serial_number } into dp_left
                               from dp in dp_left.DefaultIfEmpty()
                               join pl in process_locked_group_datas on new { sg.sku_id, sg.goods_location_id, sg.goods_owner_id, sg.serial_number } equals new { pl.sku_id, pl.goods_location_id, pl.goods_owner_id, pl.serial_number } into pl_left
                               from pl in pl_left.DefaultIfEmpty()
                               join m in move_locked_group_datas on new { sg.sku_id, sg.goods_location_id, sg.goods_owner_id, sg.serial_number } equals new { m.sku_id, m.goods_location_id, m.goods_owner_id, m.serial_number } into m_left
                               ...
                               select new
                               {
                                   ...
                                   serial_number = sg.serial_number
                               }).ToListAsync();
        ...
        public async Task<(bool flag, string msg)> Delivery(List<DispatchlistDeliveryViewModel> viewModels, CurrentUser currentUser)
        {
            ...
            var picks_g = pick_sql.AsNoTracking().GroupBy(e => new { e.goods_location_id, e.sku_id, e.goods_owner_id, e.serial_number }).Select(c => new { c.Key.goods_location_id, c.Key.sku_id, c.Key.goods_owner_id, c.Key.serial_number, picked_qty = c.Sum(t => t.picked_qty) });
            var picks = await picks_g.ToListAsync();
            var stocks = await (from stock in stock_DBSet
                                where pick_sql.Any(t => t.goods_location_id == stock.goods_location_id && t.sku_id == stock.sku_id && t.goods_owner_id == stock.goods_owner_id && t.serial_number == stock.serial_number)
                                select stock).ToListAsync();
            foreach (var pick in picks)
            {
                var s = stocks.FirstOrDefault(t => t.goods_location_id == pick.goods_location_id && t.sku_id == pick.sku_id && t.goods_owner_id == pick.goods_owner_id && t.serial_number == pick.serial_number);
                if (s == null)
                {
                    return (false, "[202]" + _stringLocalizer["data_changed"]);
                }
